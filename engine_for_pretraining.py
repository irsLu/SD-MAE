...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    ...
                    lr_schedule_values=None, wd_schedule_values=None, loss_func=None):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('min_lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_contra', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_mse', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'contra Epoch: [{}]'.format(epoch)
    print_freq = 10
    ...
    for step, (batch, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            outputs, encoder_vec, decoder_vec = model(images, bool_masked_pos)
            loss, loss_contra, loss_mse = loss_func(input=outputs, target=labels, emb_i=encoder_vec, emb_j=decoder_vec,
                                                    epoch=epoch)
            metric_logger.update(loss_contra=loss_contra.item())
            metric_logger.update(loss_mse=loss_mse.item())

            del encoder_vec, decoder_vec, labels
